...
export class MapComponent implements OnInit {
  ...
  private getLocation(): void {
    map.on('click', (e: any): void => {
      ...
      this.http.get(`https://restapi.amap.com/v3/geocode/regeo?output=json&location=${position}&key=5168b40899056c08de49fb781105d5df&radius=0&extensions=all`).subscribe((data: any) => {
        const poi = data.regeocode.pois[0];
        const info = [];
        if (poi.id === 'B00180K77K') {
          const user = this.userService.getUser();
          info.push('<div><p class="input-item">姓名 :' + user.name + '</p>');
          info.push('<p class="input-item">年龄 :' + user.age + '</p>');
          info.push('<p class="input-item">电话 :' + user.tel + '</p></div>');
        } else {
